    ...
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col font-sans">
        ...
        <main className="flex-1 flex flex-col items-center justify-center text-center px-4 pt-20">
          <h1 className="font-bold text-4xl text-[#D4AF37]">此本尊路徑尚未開啟</h1>
      ...
      <main className="mx-auto max-w-6xl px-6 pt-24 md:pt-32 pb-32">
        <section className="grid gap-12 md:grid-cols-[1.1fr_.9fr] items-center relative">
          ...
          <div className="relative z-10">
            ...
            <h1 className="mt-2 text-5xl md:text-7xl font-bold text-white drop-shadow-lg tracking-tight">{d.name}</h1>
            ...
            <Card className="mt-10 p-8 md:p-10 rounded-[2rem] border border-amber-500/20 bg-gradient-to-br from-[#0a0a0a] to-[#111] shadow-2xl relative overflow-hidden group">
              ...
              <div className="text-xs tracking-[0.24em] uppercase text-slate-500 mb-3 font-bold">對治現況卡點</div>
              <div className="text-2xl md:text-3xl text-white font-bold leading-snug mb-4">{d.oracle.question}</div>
              <div className="text-slate-400 leading-relaxed">{d.promise}</div>
            </Card>

            <div className="mt-10 flex flex-col sm:flex-row gap-4">
              <Button 
                className="h-14 px-8 font-bold tracking-widest uppercase bg-[#D4AF37] hover:bg-amber-400 text-black shadow-[0_0_20px_rgba(212,175,55,0.2)] transition-all"
                onClick={() => document.getElementById("plans")?.scrollIntoView({ behavior: "smooth" })}
              >
                查看專屬護持方案 <ArrowRight className="h-5 w-5 ml-2" />
              </Button>
              <Link href="/">
                <Button variant="outline" className="w-full sm:w-auto h-14 px-8 border-white/10 hover:bg-white/5 text-slate-300 font-bold tracking-widest uppercase backdrop-blur-sm">
                  回首頁總覽
                </Button>
              </Link>
            </div>
        ...
        <section className="mt-32 grid gap-6 md:grid-cols-3">
          {[
            ...
            <Card key={i} className="p-8 md:p-10 rounded-[2rem] border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] hover:border-amber-500/30 transition-all duration-500">
              ...
              <div className="text-2xl md:text-3xl font-bold text-white mb-5 tracking-tight">{card.sub}</div>
              ...
              <div className="flex items-start justify-between gap-6 flex-wrap relative z-10">
                <div className="max-w-xl">
                  <div className="flex items-center gap-3 mb-4">
                    <Badge className="bg-red-500 text-white border-none font-bold tracking-widest text-[10px] px-3 py-1 uppercase">限時活動</Badge>
                    <span className="text-xs tracking-[0.2em] uppercase text-green-400/80 font-bold">Special Puja</span>
                  </div>
                  <div className="text-3xl md:text-4xl text-white font-bold mb-4 tracking-tight">清明・綠度母火供<span className="text-[#D4AF37] ml-3 italic">已正式開放</span></div>
        ...
        <section id="plans" className="mt-32 scroll-mt-32">
          <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/10 pb-6 mb-12 gap-4">
            ...
            <Link href="/pay">
              <Button variant="link" className="text-slate-400 hover:text-[#D4AF37] tracking-[0.1em] px-0 h-auto font-bold">
            ...
            {d.plans.map((p) => (
              <Card key={p.id} className="p-8 md:p-10 rounded-[2.5rem] border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] hover:border-amber-500/30 transition-all duration-500 group flex flex-col relative overflow-hidden">
                ...
                <div className="flex items-start justify-between gap-4 mb-6 relative z-10">
                  <div>
                    <div className="flex items-center gap-3 flex-wrap mb-3">
                      ...
                      {p.hot && <Badge className="bg-amber-500 text-black border-none px-3 py-1 font-bold tracking-widest text-[10px] uppercase shadow-lg shadow-amber-500/20">{p.badge || "熱門選擇"}</Badge>}
                    </div>
                    <div className="text-slate-400 text-sm leading-relaxed">{p.blurb}</div>
                  </div>
                </div>

                <div className="flex items-baseline gap-1 mb-8 relative z-10">
                  <span className="text-sm text-amber-500/80">NT$</span>
                  <span className="text-4xl font-bold text-amber-500 tracking-tight">{money(p.price)}</span>
                </div>

                {p.feedback && (
                  <div className="mb-8 bg-black/60 border border-white/5 p-5 rounded-2xl text-xs italic text-slate-400 leading-relaxed relative z-10">
                    <span className="text-[#D4AF37] text-lg mr-1 leading-none font-bold">“</span>
                    {p.feedback}
                  </div>
                )}
                ...
                <div className="mt-auto relative z-10">
                  ...
                  >
                    <Button className={`w-full font-bold uppercase tracking-widest h-14 transition-colors ${p.hot ? 'bg-[#D4AF37] hover:bg-amber-400 text-black' : 'bg-white hover:bg-slate-200 text-black'}`}>
        ...
        <section className="mt-40">
          ...
          <div className="grid gap-10 md:grid-cols-[1.1fr_.9fr]">
            <Card className="p-8 md:p-12 rounded-[3rem] border border-white/10 bg-gradient-to-br from-white/[0.03] to-transparent relative overflow-hidden shadow-2xl">
              ...
              <p className="text-slate-300 italic mb-10 border-b border-white/10 pb-8 text-lg leading-relaxed relative z-10">{r.note}</p>
              
              <div className="space-y-8 relative z-10">
                {r.steps.map((step: string, idx: number) => (
                  <div key={idx} className="flex gap-6 items-start group">
                    <span className="text-4xl font-bold text-[#D4AF37]/30 leading-none group-hover:text-[#D4AF37] transition-colors duration-300">0{idx + 1}</span>
              ...
              <div className="mt-14 pt-10 border-t border-white/10 relative z-10">
                ...
                <div className="flex flex-wrap gap-3">
                  {r.used.map((u: string) => (
                    <Badge key={u} variant="outline" className="font-bold px-4 py-2 bg-black/50 border-white/10 text-slate-300 hover:border-[#D4AF37] transition-colors backdrop-blur-sm">{u}</Badge>
                  ))}
                </div>
              </div>

              <div className="mt-10 text-[10px] font-bold text-slate-500 tracking-wider uppercase relative z-10">
                資料來源參考：
                {r.sources.map((x) => (
        ...
        <section className="mt-40">
          <Card className="p-10 md:p-20 rounded-[3.5rem] border border-amber-500/20 bg-gradient-to-br from-[#050505] to-[#111] shadow-2xl text-center relative overflow-hidden">
            ...
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-12 tracking-tight relative z-10">{d.sutraMerit.title}</h2>
            
            <blockquote className="max-w-4xl mx-auto border-l-4 border-[#D4AF37] pl-8 md:pl-12 text-lg md:text-2xl text-slate-300 italic py-8 rounded-r-3xl bg-white/[0.02] leading-loose text-left mb-12 relative z-10 shadow-inner font-bold">
              {d.sutraMerit.quote.split("\n").map((line, i) => <div key={i} className="mb-2">{line}</div>)}
            </blockquote>
            
            <div className="max-w-3xl mx-auto grid gap-6 text-slate-400 leading-relaxed text-left relative z-10">
        ...
        <section className="mt-40">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-400 text-sm font-bold">真實護持者的經驗與改變</p>
          </div>
          <div className="grid gap-6 md:grid-cols-3">
            {d.testimonials.map((t, i) => (
              <Card key={i} className="p-8 md:p-10 rounded-[2.5rem] border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] transition-colors hover:border-amber-500/20 group flex flex-col h-full">
                ...
                <div className="text-sm text-slate-300 leading-loose mb-8 flex-grow">"{t.body}"</div>
                <div className="text-[10px] tracking-[0.2em] uppercase font-bold text-slate-500 border-t border-white/10 pt-6 mt-auto">
                  {t.by}
                </div>
              </Card>
            ))}
          ...
          <Accordion type="single" collapsible className="max-w-3xl mx-auto space-y-4">
            {d.faq.map((x, i) => (
              <AccordionItem key={i} value={`${i}`} className="border border-white/10 rounded-2xl bg-white/[0.02] px-2 overflow-hidden data-[state=open]:border-amber-500/30 transition-colors">
                ...
                <AccordionContent className="px-6 pb-6 text-slate-400 text-sm leading-relaxed">
                  {x.a}
                </AccordionContent>
              </AccordionItem>
            ))}
        ...
        <section className="mt-40 border-t border-white/10 pt-32 pb-10">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-400 text-sm mt-4 font-bold">探索其他本尊路徑，尋找最精準的能量對位</p>
          </div>
            ...
            {d.crossSell.map((x, i) => (
              <Link key={i} href={`/deities/${x.to}`}>
                <Card className="p-8 md:p-10 rounded-[2.5rem] border border-white/10 bg-black hover:bg-white/[0.03] transition-all group cursor-pointer hover:border-amber-500/30">
                  ...
                  <div className="text-sm text-slate-400 leading-relaxed">{x.desc}</div>
