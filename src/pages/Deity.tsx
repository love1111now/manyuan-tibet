  ...
  return (
    ...
    >
      ...
      <main>
          ...
          <div className="mt-6 grid gap-8 md:grid-cols-[1.1fr_.9fr] md:items-start">
            <div>
              ...
              <div className="mt-8 flex flex-col sm:flex-row gap-4">
                <button type="button" onClick={() => scrollToId("plans")} className="w-full sm:w-auto">
                  <Button className="h-14 w-full px-8 text-base md:text-lg font-bold tracking-[0.2em] uppercase gold-border transition-all hover:scale-[1.02] active:scale-95 shadow-xl">
                    啟動專屬修復計畫 <ArrowRight className="h-5 w-5 ml-2" />
                  </Button>
                </button>
                <Link href="/pay" className="w-full sm:w-auto">
                  <Button variant="outline" className="h-14 w-full px-8 gold-border hover:bg-primary/10 text-sm md:text-base tracking-widest">
                    檢視全站造冊總覽
                  </Button>
                </Link>
              </div>
            ...
            <Card className="gold-border bg-card/70 paper-grain overflow-hidden group">
              ...
              <div className="p-6 md:p-8 relative z-10 -mt-16 backdrop-blur-md bg-background/60 border-t border-primary/20">
                ...
                <blockquote className="mt-3 border-l-2 border-primary/50 pl-4 readable text-foreground/90 font-serif italic text-base md:text-lg leading-relaxed">
                  「{d.scripture[0]?.quote}」
                  <div className="mt-3 text-[10px] md:text-[11px] text-muted-foreground/80 tracking-widest font-sans not-italic uppercase">出處：{d.scripture[0]?.source}</div>
                </blockquote>
              </div>
            </Card>
          </div>
        ...
        {/* PROCESS */}
        <section id="process" className="mx-auto max-w-6xl px-5 md:px-8 pt-10 pb-8 scroll-mt-24">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {d.process.map((s, idx) => (
              <Card key={s.title} className="p-7 md:p-8 gold-border bg-card/70 paper-grain relative overflow-hidden group">
                ...
                <div className="mt-4 readable text-muted-foreground text-sm md:text-base leading-loose relative z-10">{s.body}</div>
              </Card>
            ))}
          </div>
        </section>
            ...
            <div className="mt-8 grid gap-8 md:grid-cols-[0.95fr_1.05fr] md:items-start">
                ...
                <div className="p-6 md:p-10 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="mt-6 flex flex-col gap-5">
                      {deityData.ritual.keyPoints && deityData.ritual.keyPoints.length > 0 ? (
                        <div className="rounded-xl border border-primary/20 bg-background/50 p-5 shadow-inner">
                          ...
                          <ul className="space-y-4">
                            {deityData.ritual.keyPoints.map((point: string, index: number) => {
                              const [title, ...descArr] = point.split("：");
                              const desc = descArr.join("：");
                              return (
                                <li key={index} className="flex items-start gap-3">
                                  <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-primary/80 shrink-0 shadow-[0_0_8px_rgba(var(--primary),0.8)]" />
                                  <div className="text-sm md:text-base leading-relaxed text-muted-foreground">
                                    {desc ? (
                                      <>
                                        <strong className="text-foreground/90 font-bold tracking-wide">{title}：</strong>
              ...
              <Card className="p-6 md:p-10 gold-border bg-card/70 paper-grain flex flex-col h-full relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="mt-3 text-sm md:text-base text-muted-foreground readable italic">
                ...
                <div className="mt-8 flex flex-col gap-8 flex-1 relative z-10">
                  {displayTestimonials.map((t: any, idx: number) => (
                    <div 
                      key={idx} 
                      className="relative pl-4 md:pl-5 border-l-[2px] border-primary/30 hover:border-primary/60 transition-colors duration-500 group"
                    >
                      ...
                      <p className="mt-3 text-sm md:text-base text-muted-foreground leading-loose readable">
                        「{t.body}」
                      </p>
                ...
                <div className="mt-10 text-center relative z-10">
                  <Link href="/proof">
                    <Button variant="outline" className="w-full h-14 text-xs md:text-sm tracking-widest font-bold hover:bg-primary/5 text-foreground hover:text-primary transition-colors border border-primary/20">
                ...
                <div className="grid gap-6 grid-cols-1 sm:grid-cols-3">
                  {d.rituals.map((r) => (
                    <Card key={r.id} className="overflow-hidden gold-border bg-card/70 paper-grain group flex flex-col border-primary/30 hover:border-primary/70 transition-all duration-500">
                      ...
                      <div className="p-5 md:p-6 flex-1 flex flex-col justify-center border-t border-border/40 bg-background/40 backdrop-blur-sm">
                        <div className="font-display text-lg md:text-xl text-primary">{r.alt}</div>
                        <div className="mt-2 text-xs md:text-sm text-muted-foreground readable leading-relaxed">{r.caption}</div>
                      </div>
                    </Card>
                  ))}
                </div>
        ...
        <section id="plans" className="mx-auto max-w-6xl px-5 md:px-8 pt-12 pb-16 scroll-mt-24">
          ...
          <div className="mb-10 p-6 md:p-8 rounded-xl border border-primary/40 bg-primary/5 relative overflow-hidden group shadow-inner">
            ...
            <p className="text-sm md:text-base text-muted-foreground readable leading-relaxed max-w-3xl">
          ...
          <div className="flex items-end justify-between gap-4 mb-8">
            <div>
              ...
              <p className="mt-3 readable text-muted-foreground max-w-prose text-base md:text-lg">
          ...
          <Card className="mb-10 p-6 md:p-8 gold-border bg-card/70 paper-grain">
            ...
            <div className="mt-5 text-[11px] md:text-xs text-muted-foreground flex items-start sm:items-center gap-2">
              <Clock className="w-4 h-4 text-primary shrink-0 mt-0.5 sm:mt-0" /> 登記完成即保留名額。若需梳理祈願內容，可透過 FB 私訊由志工溫暖陪伴。
            </div>
          </Card>
            ...
            {[...d.plans].sort((a, b) => a.price - b.price).map((p) => {
              return (
                <Card
                  key={p.id}
                  id={`plan-${p.id}`}
                  className="p-7 md:p-10 gold-border bg-card/70 paper-grain scroll-mt-28 flex flex-col justify-between transition-all duration-500 hover:border-primary/80 hover:shadow-[0_0_30px_rgba(var(--primary),0.05)]"
                >
                  <div>
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap mb-2">
                          <div className="font-display text-2xl md:text-3xl leading-snug">{p.name}</div>
                          {p.hot ? <Badge className="gold-border bg-primary text-primary-foreground font-bold tracking-widest text-[10px] px-2 py-0.5 whitespace-nowrap">系統推薦</Badge> : null}
                          {p.badge ? <Badge variant="secondary" className="bg-secondary text-secondary-foreground font-bold tracking-widest text-[10px] px-2 py-0.5 whitespace-nowrap">{p.badge}</Badge> : null}
                        </div>
                        <div className="text-xs md:text-sm text-primary/80 font-bold tracking-wide leading-relaxed">精準對位：{p.suitableFor.join("、")}</div>
                      </div>
                      
                      <div className="text-left sm:text-right w-full sm:w-auto pt-3 sm:pt-0 border-t border-border/50 sm:border-none shrink-0 mt-2 sm:mt-0">
                  ...
                  <div className="mt-8 pt-6 border-t border-border/40">
                    <div className="mb-5 p-4 bg-background/60 rounded-lg border border-primary/20 text-[11px] md:text-xs text-muted-foreground leading-relaxed shadow-inner">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button className="h-14 w-full text-sm md:text-base font-bold tracking-[0.15em] uppercase gold-border bg-primary text-primary-foreground shadow-lg transition-all hover:scale-[1.02] active:scale-95">
                              確認護持，請志工造冊 <ArrowRight className="h-4 w-4 md:h-5 md:w-5 ml-2" />
                            </Button>
                          </DialogTrigger>
                          <DialogContent className="max-w-md w-[95%] p-0 overflow-hidden bg-background border-primary/30 shadow-2xl rounded-xl">
                            
                            <div className="bg-primary/5 p-6 md:p-8 pb-5 border-b border-primary/20 text-center relative overflow-hidden">
                              <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
                                <Sparkles className="w-20 h-20 text-primary" />
                              </div>
                              <Badge className="bg-primary/10 text-primary gold-border mb-3 font-bold tracking-widest text-[10px]">志工陪伴通道</Badge>
                              <DialogTitle className="text-xl md:text-2xl font-display text-foreground/90 tracking-wide">
                                深呼吸，我們準備啟程
                              </DialogTitle>
                              <p className="text-xs md:text-sm text-muted-foreground mt-2 readable">
                            ...
                            <div className="p-5 md:p-8 space-y-6">
                              <div className="flex gap-3 md:gap-4 items-start">
                                ...
                                <div>
                                  ...
                                  <div className="mt-3 p-3 bg-background/50 border border-primary/20 rounded-md font-mono text-[11px] md:text-[12px] text-muted-foreground shadow-inner leading-relaxed">
                              ...
                              <div className="pt-3">
                                ...
                                >
                                  <Button className="w-full h-14 md:h-16 bg-primary hover:bg-primary/90 text-primary-foreground text-sm md:text-base font-bold shadow-xl transition-all active:scale-95 tracking-[0.1em] md:tracking-[0.15em] group">
                                    前往安全通道 <ArrowRight className="h-4 w-4 md:h-5 md:w-5 ml-2 group-hover:translate-x-1 transition-transform" />
                                  </Button>
                                </a>
                              </div>
                            </div>
                      ...
                      <Link href="/pay" className="w-full">
                        <Button variant="outline" className="h-12 w-full gold-border hover:bg-primary/5 transition-colors text-xs tracking-widest font-bold">
                          了解完整修復說明
                        </Button>
                      </Link>
                    </div>
                    
                    <div className="flex flex-col items-center gap-2 pt-4 mt-5 border-t border-border/30">
                      <div className="flex items-center gap-1.5 text-[9px] md:text-[10px] text-muted-foreground font-bold tracking-widest">
                        <Lock className="w-3 h-3 text-green-600/80" />
                        綠界 256-bit SSL 安全加密
                      </div>
                      <div className="flex items-center gap-2 opacity-50 grayscale scale-90">
                        <span className="text-[9px] font-bold tracking-widest flex items-center gap-1.5 uppercase">
                  ...
                  {d.faq
                    ...
                    .map((f) => (
                      <AccordionItem key={f.q} value={f.q} className="px-2 md:px-4 border-border/50">
                        <AccordionTrigger className="font-display text-left text-base md:text-lg hover:text-primary transition-colors py-5 leading-relaxed">{f.q}</AccordionTrigger>
                        <AccordionContent className="readable text-muted-foreground text-sm md:text-base leading-loose pb-5">{f.a}</AccordionContent>
                      </AccordionItem>
                    ))}
                </Accordion>
              </Card>
        ...
        <section className="mx-auto max-w-6xl px-5 md:px-8 pb-32">
          <Card className="p-7 md:p-10 gold-border bg-card/70 paper-grain">
            ...
            <div className="font-display text-2xl md:text-3xl text-foreground/90">若您的靈魂更共鳴於以下維度</div>
                ...
                return (
                  <Link key={x.to} href={`/deity/${x.to}`} aria-label={`前往 ${t.name} 頁面`}>
                    <Card className="p-6 md:p-7 gold-border bg-background/30 hover:bg-primary/10 transition-all duration-500 cursor-pointer group h-full flex flex-col justify-between">
                      <div>
                        <div className="font-display text-xl md:text-2xl transition-colors group-hover:opacity-80" style={{ color: t.themeColor.accent }}>{x.title}</div>
                        <div className="mt-3 text-muted-foreground readable text-sm md:text-base">{x.desc}</div>
                      </div>
                      <div className="mt-5 inline-flex items-center gap-2 text-xs md:text-sm font-bold tracking-widest uppercase transition-transform group-hover:translate-x-2" style={{ color: t.themeColor.accent }}>
