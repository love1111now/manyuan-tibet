...
const RITUAL_BY_KEY: Record<DeityKey, RitualBlock> = {
  yellow: {
    ...
    sources: [
      { label: "Lama Yeshe Wisdom Archive｜Dzambhala Practice for Increasing Wealth", url: "https://www.lamayeshe.com/advice/dzambhala-practice-increasing-wealth" },
      { label: "Lama Yeshe Wisdom Archive｜Water Bowl Offerings", url: "https://www.lamayeshe.com/article/chapter/13-water-bowl-offerings" },
    ],
  },
  mahashri: {
    title: "大吉祥天女：資糧圓滿儀軌",
    ...
    sources: [
      { label: "本站經典引用｜《金光明最勝王經》大吉祥天女增長財物品", url: "#/sutra" },
    ],
  },
  ganapati: {
    title: "象神：除障與智慧財商儀軌",
    ...
    sources: [
      { label: "Virtual Vinodh｜Ārya Mahā Gaṇapati Hṛdaya Dhāraṇī Sūtra", url: "https://www.virtualvinodh.com/writings/mantras/arya-maha-ganapati" },
    ],
  },
  kurukulla: {
    title: "作明佛母：懷愛與攝受儀軌",
    ...
    sources: [
      { label: "Lotsawa House｜Kurukullā Sādhana by Mipham", url: "https://www.lotsawahouse.org/tibetan-masters/mipham/kurukulla-sadhana" },
    ],
  },
};
        ...
        <section className="grid gap-10 md:grid-cols-[1.1fr_.9fr] items-start">
          <div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              ...
              <Link
                href="/"
                className="h-14 px-8 inline-flex items-center justify-center rounded-md border bg-card gold-border readable text-muted-foreground hover:text-primary transition-colors"
              >
                回首頁檢測能量
              </Link>
            </div>
        ...
        <section className="mt-12 grid gap-4 md:grid-cols-3">
          <Card className="p-7 gold-border bg-card paper-grain">
            ...
            <p className="mt-3 readable text-muted-foreground">
              不用寫很玄。用一句你看得懂、也願意承擔的話就好：例如「願我把財務節奏穩住」。
            </p>
          </Card>
          <Card className="p-7 gold-border bg-card">
            ...
            <p className="mt-3 readable text-muted-foreground">
              護持不是把責任丟出去。你可以選一件最小的事情：寫下一個決策——讓福氣開始。
            </p>
          </Card>
          <Card className="p-7 gold-border bg-card">
            ...
            <p className="mt-3 readable text-muted-foreground">
              我們不寫「保證」。比較常見的是：焦慮下降、猶豫變少，這種細微但會累積的改變。
            </p>
          </Card>
        </section>
            ...
            {d.plans.map((p) => (
              <Card key={p.id} className="p-8 gold-border bg-card hover:shadow-md transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      {p.hot && (
                        <Badge className="gold-border bg-accent/35 text-accent-foreground px-3 py-1">
                          {p.badge || "極致推薦"}
                        </Badge>
                      )}
                    </div>
                ...
                <div className="mt-4 text-[11px] text-muted-foreground leading-relaxed">
                  ...
                  <span className="block mt-2">本期活動：完成登記法事者，免費加入「煙供聖典」一次。</span>
                </div>
              </Card>
            ))}
          </div>
            ...
            <Card className="p-10 gold-border bg-card paper-grain relative">
              ...
              <div className="mt-12 pt-8 border-t border-border/50">
                ...
                <div className="flex flex-wrap gap-3">
                  {r.used.map((u) => (
                    <Badge key={u} variant="outline" className="font-normal px-4 py-2 bg-background/50 border-primary/20">
                      {u}
                    </Badge>
                  ))}
                </div>
          ...
          <Card className="p-8 gold-border bg-card paper-grain shadow-sm">
            ...
            <div className="mt-8 grid gap-6">
              {d.sutraMerit.paragraphs.map((p) => (
                <p key={p} className="readable text-muted-foreground text-lg">
                  {p}
                </p>
              ))}
            </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {d.testimonials.map((t) => (
              <Card key={t.title} className="p-8 gold-border bg-card hover:bg-accent/5 transition-colors">
                ...
                <div className="text-xs tracking-[0.2em] uppercase text-muted-foreground border-t pt-4">
                  {t.by}
                </div>
              </Card>
            ))}
          ...
          <Accordion type="single" collapsible className="max-w-4xl mx-auto space-y-4">
            {d.faq.map((x, i) => (
              <AccordionItem key={x.q} value={`${i}`} className="gold-border rounded-2xl bg-card px-4">
                ...
                <AccordionContent className="px-6 pb-6 readable text-muted-foreground text-md">
                  {x.a}
                </AccordionContent>
              </AccordionItem>
            ))}
