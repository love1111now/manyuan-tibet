    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="pb-24">
        ...
        <section className="mx-auto max-w-4xl px-5 pt-14 md:pt-20">
          ...
          <div className="mt-10"><div className="mb-6 text-center"><p className="text-xs font-bold tracking-[0.25em] text-primary">參與者回饋</p><h2 className="mt-2 font-display text-2xl md:text-3xl">共同發心之後，最珍貴的是什麼？</h2><p className="mt-2 text-xs text-muted-foreground">以下先作為回饋版位文案，正式收到信眾原話後可直接替換。</p></div><div className="grid gap-4 md:grid-cols-3">{data.feedback.map(([title,body,by]) => <article key={title} className="rounded-2xl border border-border/60 bg-card/40 p-5"><p className="text-sm font-bold text-primary">{title}</p><p className="mt-3 text-sm leading-7 text-muted-foreground">{body}</p><p className="mt-4 border-t border-border/50 pt-3 text-[10px] text-primary/70">— {by}</p></article>)}</div></div>
